execute if predicate {condition:"minecraft:any_of",terms:[{condition:"minecraft:location_check",predicate:{block:{predicates:{"minecraft:container":{items:{contains:[\
		{items:"minecraft:magma_block",count:16},\
		{items:"minecraft:fern",count:32},\
		{items:"minecraft:blackstone",count:32},\
		{items:"minecraft:grass_block",count:12}\
...
data modify entity @n[type=text_display,tag=craftdis.timeline] text set value {extra: [{bold: 1b, color: "#14DEA8", text: "200 000 000 BC", obfuscated: 0b}], text: ""}
...
data merge storage craftdis:craftdis {target:{x:0,y:64,z:0,dimension:"craftdis:prehistory",dimension_name:"200 000 000 BC"}}
